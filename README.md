...
- 处理本地商品资料包时，调用 `product_precheck_package`；必填校验通过后，先用商品中文名称调用 `product_check_name_duplicate`。
- 如果 `product_check_name_duplicate` 返回 `exists: true` 或 `blocking: true`，中断该商品上传和创建；多商品并行 worker 要把资料包路径、商品中文名称和 `duplicates` 返回给总控。
...
- 多商品并行创建时，总控先调用一次 `product_runtime_self_check` 和 `product_auth_status`，并把验收结论传给 worker；worker 不应在读取到缓存 token 后再重复做 self-check。
...
- 创建前同名查重：`0.3.18` 起，Launcher 和 Runtime Proxy 预声明 `product_check_name_duplicate`；Product MCP `0.1.15` 起在必填校验通过后、上传前按商品中文名称查重，查到同名商品则中断上传和创建。
- 并发 self-check 复用：`0.3.19` 起，Launcher 和 Runtime Proxy 会短期复用成功的 `product_runtime_self_check` 结果并合并进行中的并发 self-check；总控可用 `forceRefresh: true` 显式要求完整刷新。
- Token 诊断与刷新收敛：`0.3.20` 起，Skill 会要求 AI 使用 Product MCP `0.1.18` 返回的 `transport.auth` 判断业务工具是否复用了 daemon 缓存；刚从 Chrome 读取的 token 如果仍被后端 401/403 拒绝，不会在同一个工具调用里再次唤起 Chrome，而是清空缓存并返回错误，等待用户刷新或重新登录 ERP。
...
- For local product packages, call `product_precheck_package`; after required-field validation passes, call `product_check_name_duplicate` with the Chinese product name.
- If `product_check_name_duplicate` returns `exists: true` or `blocking: true`, stop upload and creation for that product. In parallel multi-product creation, the worker must return the package path, Chinese product name, and `duplicates` to the controller.
...
- In parallel multi-product creation, the controller calls `product_runtime_self_check` and `product_auth_status` once and passes that verification to workers; workers should not repeat self-check after reading a cached token.
...
- Pre-create duplicate-name gate: starting in `0.3.18`, the Launcher and Runtime Proxy predeclare `product_check_name_duplicate`. Starting with Product MCP `0.1.15`, after required-field validation and before upload, AI Agents check the Chinese product name and stop upload/create when a same-name product exists.
- Concurrent self-check reuse: starting in `0.3.19`, the Launcher and Runtime Proxy reuse successful `product_runtime_self_check` results for a short window and coalesce in-flight concurrent self-checks. Controllers can pass `forceRefresh: true` when a full refresh is explicitly required.
- Token diagnostics and refresh convergence: starting in `0.3.20`, the Skill tells AI Agents to use Product MCP `0.1.18` `transport.auth` diagnostics to tell whether business tools reused the daemon cache. If a freshly read Chrome token is still rejected by the backend with 401/403, Product MCP clears the cache and returns the error without reopening Chrome again inside the same tool call; the user should refresh or log in to ERP before retrying.
...
2. Calls the ERP backend directly from the local bridge for lookups, uploads, creates, and detail checks.
3. Invalidates the token cache and reads it once again on stale-token 401/403, unless the token was just read from Chrome; in that fresh-token case it returns the backend error without immediately opening Chrome again.
4. Returns redacted `transport.auth` diagnostics on business results so AI can tell whether `tokenSource` was `cache` or `chrome` without seeing the token value.

...
6. 对 Codex 暴露 Product MCP 的业务工具，以及代理层的 `product_runtime_self_check`、`product_runtime_status` 和 `product_runtime_refresh`。
...
代理运行期间会按间隔检查 Product MCP Git 更新；如果发现 Product MCP checkout 或 bridge 配置变化，会只重启子运行时并通知工具列表变化，Codex thread 上下文继续保留。`product_runtime_self_check` 会主动执行一次自检/自愈，并读取子运行时的 `product_bridge_config_status` 来确认实际生效配置。
...
6. Exposes Product MCP business tools plus the proxy-level `product_runtime_self_check`, `product_runtime_status`, and `product_runtime_refresh` tools to Codex.
...
While the proxy is running, it periodically checks for Product MCP Git updates. If the checkout or bridge config changes, it restarts only the child runtime and notifies Codex that the tool list changed. The current Codex thread context is preserved. `product_runtime_self_check` actively performs one self-check/self-heal pass and reads the child runtime's `product_bridge_config_status` to confirm the effective config.

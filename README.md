...
- 更新后验收：AI Agent 应调用 `product_runtime_status`、必要时调用 `product_runtime_refresh`、再调用 `product_auth_status` 自行确认运行时和配置是否生效。不要要求普通用户读取 hash、判断版本、运行命令或自行验证 MCP 状态。
...
- Post-update acceptance: the AI Agent should call `product_runtime_status`, call `product_runtime_refresh` when needed, then call `product_auth_status` to confirm the runtime and config are active. Do not ask ordinary users to read hashes, judge versions, run commands, or verify MCP status themselves.

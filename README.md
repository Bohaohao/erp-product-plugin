...
- Product MCP 运行时更新：用户保留当前 thread 上下文。若 MCP 进程需要重新加载，用户可以关闭并重新打开同一个 thread，或重启 Codex 后回到同一个 thread。插件启动时会自动拉取或复用最新可用的 `product-mcp`。
...
- Product MCP runtime updates: the user keeps the current thread context. If the MCP process needs to reload, the user may close and reopen the same thread, or restart Codex and return to the same thread. The launcher automatically pulls or reuses the latest available `product-mcp`.
